...
public class DeadPig extends Item {

   public DeadPig(int id) {
      super(id);
      this.setMaxStackSize(1);
      this.setCreativeTab(CreativeTabs.tabMisc);
      this.setUnlocalizedName("deadPig");
      this.setTextureName("Hooks".toLowerCase() + ":deadPig");
      this.setMaxDamage(1);
   }
   ...
   public boolean onItemUse(ItemStack itemStack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ) {
      ...
      if(blockId == Hooks.hook.blockID) {
         if(world.isAirBlock(x, y - 1, z)) {
            world.setBlock(x, y, z, Hooks.hookedPig.blockID);
         ...
         world.setBlock(x, y, z, Hooks.spitPig.blockID, blockMeta, 2);
         player.inventory.consumeInventoryItem(itemStack.itemID);
      } else {
         ...
         if(world.getBlockId(x + 1, y, z) == Hooks.spitStick.blockID) {
            world.setBlock(x + 1, y, z, Hooks.spitPig.blockID, world.getBlockMetadata(x + 1, y, z), 3);
         } else if(world.getBlockId(x - 1, y, z) == Hooks.spitStick.blockID) {
            world.setBlock(x - 1, y, z, Hooks.spitPig.blockID, world.getBlockMetadata(x - 1, y, z), 3);
         } else if(world.getBlockId(x, y, z - 1) == Hooks.spitStick.blockID) {
            world.setBlock(x, y, z - 1, Hooks.spitPig.blockID, world.getBlockMetadata(x, y, z - 1), 3);
         } else if(world.getBlockId(x, y, z + 1) == Hooks.spitStick.blockID) {
            world.setBlock(x, y, z + 1, Hooks.spitPig.blockID, world.getBlockMetadata(x, y, z + 1), 3);
         }
